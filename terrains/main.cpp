...
const int width=900, height=9*width/16;
...
int main(int, char**){
    ...
    window.set_title("Terrain Sample");
...
void genTerrainMesh() {
    ...
    for(int j=0; j<n_height-1; ++j) {
        for(int i=0; i<n_width-1; ++i) {
			...
			indices.push_back(j * n_width + (i + 1));
			/// Triangle 2
			indices.push_back(j * n_width + (i + 1));
			indices.push_back((j + 1) * n_width + i);
			indices.push_back((j + 1) * n_width + (i + 1));
        }
    }
...
void drawTerrain() {
    ...
    Mat4x4 P = perspective(100.0f, width / (float)height, 0.01f, 10.0f);
